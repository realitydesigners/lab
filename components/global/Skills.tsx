...
const ServiceSection = () => {
    return (
        <div className="flex w-full flex-col justify-between px-4 py-20 text-white md:px-40 lg:flex-row ">
            <h2 className={`${monomaniac.className} mb-4 text-3xl uppercase `}>
                Our Favorite
                <br />
                Technologies
            </h2>
            <ul className="grid grid-cols-2 gap-1 lg:grid-cols-5">
                {Skills.map(({ label, icon }) => (
                    <li
                        key={label}
                        className="flex items-center border  border-gray-600/50"
                    >
                        <div className="flex min-h-10 min-w-10 items-center pl-2 pr-2 ">
                            {getIcon(icon)}
                        </div>
