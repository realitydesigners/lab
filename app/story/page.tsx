	...
	return (
		<>
			<div className="w-full relative h-[90vh] justify-center flex items-center">
				<div className="lg:w-1/2 w-full h-[555px] md:h-[800px] lg:h-full absolute  lg:right-0 ">
					<Spline scene="https://prod.spline.design/jEsawKZEsxEwrEN5/scene.splinecode" />
				</div>
				<div className="absolute lg:left-40 left-0  w-full p-8 lg:p-0 md:w-1/2 lg:w-1/3">
					<h1
						className={`${monomaniac.className} text-gray-200  leading-0 uppercase text-8xl lg:text-9xl  mb-6`}
					>
						We Design Worlds
					</h1>
					<p className=" text-xl text-gray-300 leading-6  w-full lg:w-3/4">
						In a world full of challenges, it calls for visionaries —
						individuals who don't just recognize what is essential today but are
						also dedicated to sculpting the solutions of tomorrow.
					</p>
				</div>
			</div>

			<div className="flex h-auto py-20 flex-col items-center justify-center">
				<div className="w-full flex flex-col  justify-between  items-center gap-12 lg:px-40 px-6">
					<p className="text-3xl leading-7 text-gray-300  w-full lg:w-2/3">
						We're on a path less traveled, demanding resilience and an
			...
			<div className="flex h-auto flex-col items-center justify-center py-32">
				<div className="w-full flex flex-col lg:flex-row justify-between  items-center gap-12 lg:px-40 px-6">
					<h1
						className={`${monomaniac.className} lg:w-1/2 w-full text-gray-200 text-6xl uppercase pr-20`}
					>
	...
	return (
		<div>
			<div className="w-full flex justify-center items-center">
				<h2
					className={`${monomaniac.className} text-gray-200 w-3/4 lg:w-1/2 text-center text-5xl md:text-7xl uppercase`}
				>
...
const ServiceSection = () => {
	...
	return (
		<div className="flex flex-row flex-wrap px-4 md:px-40 py-10 text-white w-full ">
				...
				>
					{Services.map(({ label, icon, desc }) => (
						...
						>
							{getIcon(icon)}
							<div
								className={`${monomaniac.className} uppercase leading-none text-xl ml-2`}
							>
								{label}
							</div>
						</li>
			...
			{isDetailVisible && (
				<div
					className="flex-grow lg:w-3/4 transition-opacity duration-200 ease-in-out overflow-hidden lg:-16 p-8 mt-4 ml-0 border border-gray-600/25 lg:ml-4 lg:mt-0 h-[80vh]"
					style={{
				...
				>
					<h2
						className={`${monomaniac.className} leading-none uppercase text-4xl lg:text-5xl text-gray-200 mb-4`}
					>
						{activeService}
					</h2>
